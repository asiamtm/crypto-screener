...
# Load Binance API keys from environment (Streamlit secrets or local)
...
# Disable ping to bypass Streamlit Cloud restriction
...
async def fetch_spot_change(client, symbols):
    out = {}
    for s in symbols:
        try:
            data = await client.get_ticker(symbol=s)
            out[s] = float(data.get("priceChangePercent", 0)) / 100
        except Exception as e:
            print(f"Error fetching spot data for {s}: {e}")
            out[s] = np.nan
    return out
...
async def load_and_screen():
    ...
    try:
        if not BINANCE_API_KEY or not BINANCE_API_SECRET:
            st.warning("🔐 No API keys found. Running in anonymous spot-only mode.")
    ...
    changes = await fetch_spot_change(client, syms)
    ...
    for s, df in zip(syms, ohlcvs):
        spot_change = changes.get(s, np.nan)
        if df.empty or len(df) < ATR_LEN or np.isnan(spot_change):
            continue
    ...
    for score_level, label in zip([3, 2, 1], ["🚨 FULL PRE-DIP 🚨", "⚠️ NEAR-DIP", "🔥 WARM-DIP"]):
        ...
        if not subset.empty:
            ...
            for _, row in subset.iterrows():
                with st.expander(f"{row['Symbol']} | Δ {row['PriceChange']:.2%}"):
